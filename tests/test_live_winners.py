...
def test_競りの旗は門やない():
    """**旗は立てるが段は動かさん**(2026-09-24)。

    前向き台帳22件で「競りが付いた玉は終値が中央値の0.81〜0.99倍まで
    上がる(上限は0.66〜0.75倍)」と出た。せやが n=8 や。
    ここで段に混ぜたら、**規則を作ったデータで採点する**ことになるし、
    「入札が付いた玉は本当に取れんのか」が二度と測れんくなる。
    """
    src = (ROOT / "core" / "watchlist.py").read_text(encoding="utf-8")
    seg = src[src.index("def live_winners"):]
    seg = seg[:seg.index("def winners")]
    assert '"競り"' in seg, "競りの旗が無い"
    # **段の式に入札数が入っとらんこと**を確かめる。ここが門になったら測れん
    i = seg.index('d["段"] = np.where')
    stage = seg[i:seg.index("return d.sort_values")]
    assert "入札" not in stage, "入札数が段の門になっとる(n=8では早い)"


def test_入札欄の無い玉を競り無しに化かさん():
    d = pd.DataFrame({
        "いま買える": ["○", "○", "○"],
        "勝ち語": [5, 5, 5],
        "欠陥": [None, None, None],
    ...
    got = np.where(d["入札数"].isna(), "—",
                   np.where(d["入札数"] > 0, "🔥 競り中", "🤫 入札なし"))
    assert list(got) == ["🤫 入札なし", "🔥 競り中", "—"], list(got)
    # **欠測は「—」。0(=誰も入札しとらん)と混ぜたらあかん**
    print("OK 旗: 0=入札なし / >0=競り中 / 欠測=— を混ぜとらん")
...
if __name__ == "__main__":
    ...
    test_数える時も買える玉だけ()
    test_競りの旗は門やない()
    print("OK 競り: 旗は立つが段の門にはなっとらん")
    test_入札欄の無い玉を競り無しに化かさん()
    print("\n検定ぜんぶ通った")
