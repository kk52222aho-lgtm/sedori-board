...
if S.CLOUD:
    _m = S.snap_meta()
    _head = (f"**{_m.get('作成', '?')[:16]} の断面**(ローカルで "
             "`python export_snapshot.py` を回して push した時点)。")
    _tail = ("買い/売りアラートはその時刻のもので、**リアルタイムやない**。"
             "進行中個体の検品ボタンだけは今この場でヤフオクを見に行く。")
    _age = S.snap_age_h()
    # 🚨 **古い断面は「情報」やのうて「警告」や。** 青い枠で時刻を出しとるだけ
    # やと、80時間前の写真も今朝の写真も同じ顔で出る。フリマは実測で
    # **73%が27時間で消える**んやから、丸1日過ぎた「いま買える」は
    # もう買えん玉の一覧や(2026-09-21)
    if _age is not None and _age > 36:
        st.error(
            _head + f" **その断面は {S._human(_age)} 前や。**\n\n"
            "**この盤に出とる「いま」は全部その時刻の「いま」で、今の話やない。**"
            "フリマは実測で73%が27時間で消えるから、"
            "「🛒 いま買える」はもう買えん玉が大半や。"
            "ヤフオクの買いアラートは終了時刻で切れるので **0件に見える**——"
            "出物が無いんやのうて、**断面が古いだけ**や。\n\n"
            "直すには家で `python export_snapshot.py` を回して push する。", icon="🚨")
    else:
        st.info(_head + _tail)
st.caption(
...
# ------------------------------------------------------------------ いま買える
# 🚨 **名前で引いても、名前を取り違えたら同じ事故になる**(2026-09-17)。
# タブを番号やのうて名前で引くようにしたのは「途中に1枚差し込んだら後ろが
# 全部ズレる」を防ぐためやったが、この2枚は**キーそのものが入れ替わっとった**——
# 買い目(BL.load)が「🎯 勝てる商品」に、型番の見立てが「🛒 いま買える」に出とった。
# 番号ズレと違うて、見出しの脇の節コメントだけが正しい状態で静かに残る。
# **節コメントと `with T[...]` のキーは必ず一致させること。**
with T["🛒 いま買える"]:
    bl = BL.load()
    if bl.empty:
        ...
        # 🚨 **「いま買える玉 N件」は走査時刻の話や。** 73%が27時間で消える面で、
        # 丸1日過ぎた一覧を件数だけ太字で出したら嘘になる(2026-09-21)
        _scan_age = S.snap_age_h()
        if _scan_age is not None and _scan_age > 27:
            st.error(
                f"**この {len(live)}件 は {S._human(_scan_age)}前の走査や"
                f"(実測の消滅窓の {_scan_age / 27:.1f}倍)。** "
                "Yahoo!フリマは**27時間で73%が消える**面やと実測しとる"
                "(2026-08-17)。その先どう減るかは測っとらんから件数は出さんが、"
                "**大半はもう無い**と思て動くこと。"
                "**「売切」列もこの走査時点の話で、今の在庫やない。** "
                "家で `export_snapshot.py` を回し直すまでは、"
                "リンクを開いて生きとるか確かめてからにする。", icon="🚨")
        st.caption(
...
with T["🎯 勝てる商品"]:
    ...
    with left:
        ...
        if lw.empty:
            ...
            st.caption(
                "🚨 **2026-09-24: 入札の旗が意味するんは「買えるか」だけで"
                "「得か」やない。** 門を通した落札 **17,898行**を入札数で割ったら、"
                "終値/中央値は 即決1.04 / 1本1.00 / 2本0.96 / 20本超0.97 で"
                "**平ら**やった。買い線(中央の0.70)以下に落ちる割合も3〜12%で"
                "入札数と関係ない。"
                "生の落札で見えた勾配(1本0.54→20本超0.91)は**部品とアクセサリの"
                "混入**で、本体だけにしたら消える。"
                "  /  "
                "**🔥競り中** = 線の内側に居ても終値が中央値の0.98倍まで上がるんで"
                "線の外へ出る。**🤫入札なし** = 落とせる。"
                "せやが盤の実測で、**買い線の内側で入札ゼロの9件は9件とも欠陥あり**"
                "やった(Fisher 両側 p=0.026)。欠陥なしの9件は9件とも競り中で、"
                "**交わりはゼロ**。"
                "**取れる玉と欲しい玉が重ならん、いうんがこの筋の形。** "
                "旗は門やない。前向き台帳に入札数が溜まるまでは切らん。")
            show = lw if st.checkbox("見送り(3点以下)も出す", value=False) else buyable
            if show.empty:
                st.info("価格は合っとるが質が足りん玉ばかり。下のチェックで中身を見れる。")
            else:
                st.dataframe(
                    show[[c for c in ["段", "いま買える", "競り", "勝ち語",
                                      "想定純利",
                                      "現在価格", "上限まで", "残り時間h",
                                      "勝ち語の中身", "欠陥", "title", "url"]
                          if c in show]],
    ...
    # 🚨 **競りが付かんかった玉で見込みを立てとらんか**(2026-09-22)。
    # 実弾GO 9型番の落札773行で測ると、**入札1以下の玉は家族の中央の
    # 1.06倍**で売れとる(即決の値付け)。入札2以上は0.95倍。
    # つまり普通は「競り無し=高い」で、**安いのに誰も競らんかった玉は
    # 入札1以下の15%しかない**。その少数派がうちの生存に混じっとる。
    _thin = view[(view.get("競り無し", False) == True)  # noqa: E712
        ...
        column_config={
            ...
            "束の疑い": st.column_config.CheckboxColumn(
                "束", help="生存2件以上が全部同じ日に終わっとる。"
                           "1出品者のまとめ放出なら機会は1件や"),
            "単発": st.column_config.CheckboxColumn(
                "単発", help="生存が1件。期待粗利=件数×中央値の式は"
                             "「また同じ玉が出る」を仮定しとるが、1件はその外や"),
            "生存入札中央": st.column_config.NumberColumn(
                "競り", help="生存した玉に付いた入札数の中央値"),
            "生存粗利中央": st.column_config.NumberColumn(
                "1件あたり", format="¥%d",
                help="生存した玉1個から出る粗利の中央値。期待粗利180dは"
                     "これ×生存件数や。**薄い玉を何回も回せば合計は越えられる**"),
            "薄利": st.column_config.CheckboxColumn(
                "薄", help="1件あたりが¥3,000未満。候補に入れる床(GROSS_MIN)と"
                           "同じ数字や。実弾GOを1件あたりで並べたら下2本が"
                           "**取引の47%を食うて金は18%**やった"),
            "競り無し": st.column_config.CheckboxColumn(
                "競無", help="生存した玉の入札が中央1以下。実弾GO9型番の落札773行で"
                             "測ると入札1以下は普通**家族の中央の1.06倍**で売れとる"
                             "(即決の値付け)。安う買えとるのに誰も競らんかったんは"
                             "**相手が情報を持っとる合図**や"),
        })
...
with T["🌏 レーン比較"]:
    ...
    if lanes.empty:
        ...
        CFG = {
            ...
            "最終確認": st.column_config.TextColumn(
                "最終確認", help="**その玉を最後に見た時刻。** fleet_watch は"
                                 "1時間おきに掃いとって、直近の掃きに居らんかった"
                                 "玉は「いま買える」から落としとる"
                                 "(2026-08-31、売れた MARANTZ SA-10 が翌日も"
                                 "「いま買える1件」で出とった)"),
