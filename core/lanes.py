...
## 2種類のレーンは数字の温度がちゃう
- **輸出(相場)** — 1行=1機種。中央値の推定で、目の前に玉は無い。年に何台出るかまで読む
- **国内(現物)** — 1行=**いま出とる1個**。URLを開けばそこにある。売れたら消える
...
def _export_from_souba() -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        "品": d["機種"], "レーン": "日本 → eBay US", "種別": "輸出(相場)",
        "仕入面": "ヤフオク等(p25)", "仕入値": buy,
        "売面": "eBay US 実売", "売値": d["out_中央"],
...
def _domestic_from_souba() -> pd.DataFrame:
    ...
    for fname, market in FLIP_MARKET.items():
        ...
        rows.append(pd.DataFrame({
            ...
            "レーン": f"{market} → 国内出口", "種別": "国内(現物)",
            "仕入面": market, "仕入値": d["price"],
            "売面": "国内相場(中央)", "売値": d["median"],
