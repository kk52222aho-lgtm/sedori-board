...
def freshness() -> pd.DataFrame:
    ...
    # 🚨 **「いま買える玉」を名乗る源が鮮度の門の外に居った**(2026-09-17)。
    # live_winners.csv は**進行中**のヤフオクを並べる表や。せやのに鮮度を
    # 誰も見とらんくて、走査が **40日** 古いまま「今すぐ買える 7件」と
    # 出しとった——8月に終わった玉ばかりや。
    # 落札の表と違うて、この表は**古い=間違い**になる(進行中やなくなるから)。
    # 定期実行は無いが、7日先まで見る走査やから3日で全部終わっとる。
    # 周期24h(= FRESH_DEAD の3倍で🚨停止)を置いて、止まったら黙らんようにする。
    add("進行中の勝ち語(いま買える玉)", SOUBA / "data/camera/live_winners.csv",
        "「🎯勝てる商品」の『いま買える玉』はこれ。"
        "古いと**終わった玉を「今すぐ買える」と出す**", 周期h=24, 土台=True)
