...
def build_master() -> pd.DataFrame:
    ...
    for niche, cfg in S.NICHES.items():
        ...
        if not ver.empty:
            ...
            agg = ver.groupby("family").agg(
                ...
                # 🚨 **生存した玉に競りが付いとったか。**(2026-09-22)
                # 実弾GO 9型番の落札773行で測ると、**入札1以下の玉は
                # 家族の中央の1.06倍で売れとる**(即決の値付け)。
                # 入札2以上は0.95倍。つまり普通は「競り無し=高い」や。
                # 逆に**入札1以下やのに中央を大きく下回る玉は15%しかない**。
                # その少数派が、うちの「生存」に混じっとる:
                #   SAL70200G2 ¥89,000 = 中央の0.80倍・入札1(生存これ1件だけ)
                #   SEL1224G   ¥95,000 = 中央の0.73倍・入札1(同上)
                # **安く買えた=悪い知らせ**の形(2026-08-07に紙上勝ち6件を
                # 検品して0/6やったんと同じ)。等級は黙って動かさん——旗を立てる。
                生存入札中央=("bids", lambda s: pd.to_numeric(
                    s[ver.loc[s.index, "is_keep"]], errors="coerce").median()),
            )
        else:
    ...
    # **生存が1件** = 見込みを1個の玉で立てとる。件数×中央値の式は
    # 「また同じ玉が出る」を仮定しとるが、1件はその仮定の外や
    df["単発"] = df["生存"] == 1
    # **競りが付いとらん。** 入札1以下が普通は「即決=高い」やのに、
    # 安う買えとる = 相手が情報を持っとる合図([[逆選択]])
    df["生存入札中央"] = pd.to_numeric(df.get("生存入札中央"),
                                      errors="coerce")
    df["競り無し"] = (df["生存"] >= 1) & (df["生存入札中央"] <= 1)
    # 🚨 **1件あたりの床が無かった**(2026-09-22)。等級の閾値は
    # 「家族の合計 ¥10,000/180日」だけで、**薄い玉を何回も回せば越えられる**。
    # 実弾GOを1件あたりで並べたら、下2本(P900 ¥1,797 / SEL2470Z ¥1,750)が
    # **取引の47%(16/34)を食うて、金は18%**やった。
    # 床は `spread_camera.GROSS_MIN`(¥3,000)と同じ数字を借りる——
    # 候補に入れる時に使うとる床やから、こっちで発明せん。
    df["薄利"] = (df["生存"] >= 1) & (df["生存粗利中央"] < 3000)
...
def live_winners() -> pd.DataFrame:
    ...
    # 片方だけで貼ったらあかん。
    # 🚨🚨 **掛け算の項を数え直した**(2026-09-23、同じ日に2回踏んだ)。
    # 1回目は「買えるか」を足した。せやが**元の道具はもう1項見とった**:
    #
    #     live_winners.py: go = [r for r in rows
    #                            if r["勝ち語"] >= min_score and **not r["欠陥"]**]
    #
    # 盤はその `欠陥` を見とらんかった。唯一「👍 買える」が立っとった玉は
    # **欠陥=欠品**やった。**盤が、データを作った道具より甘かった。**
    #
    # **項を1つ足して満足したらあかん。作った側が何で絞っとるかを読んで
    #   全部数える。** ここは3つや: 値段(買い線の内) × 質(勝ち語) × 欠陥なし。
    # 🚨 **4項目めが見えた(2026-09-24)。やが門にはせん。**
    # 🚨🚨 **同じ日に意味を訂正した。**門を通した落札17,898行では、終値/中央値は
    # 入札数によらず 0.96〜1.04 で平ら。下の「0.81〜0.99倍まで上がる」は
    # **本体だけにしたら消える**(生の行は部品とアクセサリが混じっとった)。
    # 旗が言えるんは「落とせるか」だけ。「安う終わるか」やない。
