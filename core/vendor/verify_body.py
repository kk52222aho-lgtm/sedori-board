...
UA = "Mozilla/5.0 (compatible; souba-league/1.0; personal research)"
...
FORWARD_RE = re.compile(
    ...
    # 2026-08-24、撃墜12件を読み返したら**7件が定型文の誤爆**やった(誤爆率64%)。
    # 買い線と比べると PW3360「未使用品」¥86,000(+¥8万相当)と
    # WT1800「8日保証・未使用に近い」¥474,100(+¥20万相当)を捨てとった。
    # 漏れとったのは**欠陥語がカテゴリ名として使われる形**や:
    #   「※**ジャンク品や現状販売品など**、上記状態欄に説明がある場合は…」
    #   「※**ジャンクの商品については**、記載内容以外の不具合が…」
    # 「〜品や」「〜の商品については」は**この個体の話やない**。
    # **免責の見出し。** HIOKI BT3554 ¥253,000 は撃墜語3つが全部
    # 【返品・返金対象外の項目】の箇条書きやった(2026-08-24)
    r"ジャンク品や|ジャンク品・|ジャンクの商品|現状販売品|"
    r"以下のもの|下記のもの|以下の商品|以下の点|下記の点|初期不良の対象外|"
    r"保証対象外|保証の対象外|免責|以下の事項|下記の事項|"
    r"対象外となります|返品はお受けできません|返品・返金は")
CLOSING_RE = re.compile(
    # 出品者のFAQ。「■「引き取り限定商品」の発送は**出来ますか**？・行っておりません」
    # が丸ごと実在の制約として読まれとった(2026-08-24)。質問文はこの個体の話やない。
    r"出来ますか|できますか|ますか？|ますか\?|"
    # 「…必ず状態欄を**ご確認ください**」で終わる注意書き
    r"ご確認ください|ご確認下さい|優先させていただきます|"
...
# **免責の見出しは字数で切ったらあかん。** HIOKI BT3554 ¥253,000 の
# 【返品・返金対象外の項目】は箇条書きが長うて、最後の「・ジャンク品」が
# DISCLAIMER_SPAN=120 の外に出とった(2026-08-24)。一律に窓を広げると、
# 2026-08-11 に踏んだ「実在の欠陥を並べた箇条書きを丸ごと消す」に戻る。
# **見出し型だけ、次の見出し(■◆★●【)まで**を塞ぐ。
DISCLAIMER_HEAD_RE = re.compile(
    r"返品・返金対象外|返金対象外|返品対象外|対象外の項目|"
    r"返品・保証|返品規定|返品について|返品に関して|"
    r"保証・返品|キャンセル・返品|免責事項")
DISCLAIMER_HEAD_MAX = 400   # 見出しが閉じんかったときの上限
...
# **FAQの「回答」側。** 後ろ向きマスクは質問文しか塞げん(2026-08-24に発覚):
#
#   ■離島への発送は可能ですか？ ・可能です(**引き取り限定商品**、ヤマト家財便一部地域不可)
#
# 撃墜語は質問より**後ろ**の回答文にあるので、CLOSING では絶対に届かん。
# HIOKI PW3360「未使用品」¥86,000(買い線¥165,710)を、これ一つで捨てとった。
#
# 🚨 **「？で終わる文の先を塞ぐ」は絶対にやったらあかん。** 出品者のFAQには
# この個体の話をするものもある:
#
#   ■動作は確認していますか？ ・通電確認のみです   ← **これは実在の申告や**
#
# せやから**物流・取引の質問に限る**。送料や梱包の答えが個体の欠陥を述べる
# ことはない。質問文の中に物流語があるものだけを拾う。
FAQ_SHIP_RE = re.compile(
    r"[^。．\n■◆★●]{0,40}"
    r"(発送|送料|配送|同梱|梱包|支払|振込|返品|返送|領収|明細|"
    r"引取|引き取り|離島|落札後|取引|キャンセル|値下げ|値引き|"
    r"取り置き|取置|海外|転送|クレーム)"
    r"[^。．\n■◆★●]{0,40}"
    r"(ますか|ですか|でしょうか|でしょか)[？?]")
# 回答は次の見出し(■◆★●)まで。届かんかったらここで切る。
# 120字にしたら次のFAQまで食う出品があったので短めに取る。
FAQ_ANSWER_SPAN = 90

# 明示的な否定。「カビはありません」を欠陥として拾わんため。
# 「カビくもり**なく**クリアー」を欠陥として拾っとった(2026-08-07)。
# 人手で生存確定させた SAL70200G2 がこれで撃墜されとった。連用形の否定を足す。
# **ぼかした否定と強めた否定**(2026-08-24、②層44件の人手ラベルで判明):
#   「クモリやカビ等も**ほとんど見当たりません**」    AG-CX350 ¥350,000
#   「アタリへこみなどの大きなダメージが**一切ない**」 AG-DVX200 ¥218,800
# どっちも綺麗やと言うとるのに、欠陥ありと読んどった。
NEGATION_RE = re.compile(
    r"[^。]{0,12}?(?:ほとんど|殆ど|ほぼ)(?:見当たり|見受けられ|あり|ござい)ませ|"
    r"[^。]{0,12}?一切(?:ない|ありませ|ございませ|無く|なく)|"
...
OFFSET_RE = re.compile(
    ...
    # 「ホコリの混入ありますが、**撮影に影響のあるものではございません**」
    # (EOS C200 ¥298,000、2026-08-24)。既存は「影響ありませ」しか見てへん
    r"影響(?:の|が)ある(?:もの|よう)?で?は?(?:ござい|あり)ませ|"
    r"実用(?:上|に)(?:は)?(?:問題|支障)(?:は)?(?:あり|ござい)?ませ)")
NEGATION_SPAN = 16  # 語の直後この字数以内に否定があれば無視
# 「ジャンク品**と記載のある**もの」「難あり/ジャンク品**で出品されている**もの」=
# 品目クラスへの言及であって、この個体の申告やない。返品規定の常套句や。
REFERENCE_RE = re.compile(
    # **②層(撃墜側)を語ごとに読んだら、上位7件のうち5件が誤爆やった**
    # (2026-08-24)。①層(マスク救済)は14/14で素通しゼロやったのに、
    # 撃墜側は較正できてへんかった。**通した側だけ測っても半分**の実例や。
    # 漏れとった言い回し:
    #   「ジャンク**での出品商品につきましては**保証しません」
    #     既存は「で出品」を見とったが「**での**出品」で外れる
    #   「ジャンク品**は**NC/NRでお願い致します」= 出品者の取引方針
    #   「動作未確認…**と予め記載している商品については適用外**」
    #     「と記載」の間に「予め」が入って14字の窓から出とった
    r"[^。]{0,20}?(?:[とで]の?(?:記載|表記|明記|出品|され)|"
    r"扱いの(?:商品|もの)|品?(?:の|は)返品|品?(?:の|は)?ご返品|"
    r"品?は\s*(?:NC|ノークレーム)|"
    # 「〜については」で始まる条件節は、この個体やのうて種類の話や
    r"(?:品|もの|商品)?(?:に)?(?:つきまして|ついて)は)")
...
def _disclaimer_zones(body: str, closing: bool = True) -> list[tuple[int, int]]:
    ...
    # 免責の見出しから、次の見出しまで
    for m in DISCLAIMER_HEAD_RE.finditer(body):
        hi = min(len(body), m.end() + DISCLAIMER_HEAD_MAX)
        for j in range(m.end(), hi):
            if body[j] in "■◆★●【":
                hi = j
                break
        zones.append((m.start(), hi))
    # 物流FAQの**回答**を塞ぐ。次の見出しが来たらそこで止める
    for m in FAQ_SHIP_RE.finditer(body):
        hi = min(len(body), m.end() + FAQ_ANSWER_SPAN)
        for j in range(m.end(), hi):
            if body[j] in "■◆★●":
                hi = j
                break
        # **質問文も一緒に塞ぐ。** 構造判定(closing=False)では後ろ向き
        # マスクが効かんので、質問の中の「引き取り限定商品」が生き残る。
        # 物流の質問文そのものも、この個体の申告やない。
        zones.append((m.start(), hi))
    if not closing:
        return zones
...
def _is_live_hit(body: str, m: re.Match, zones: list[tuple[int, int]]) -> bool:
    ...
    **同じ文の中**を探す形にする(。をまたいだら別の話やから見に行かん)。
    ...
    # **可用の明言が前の文にあることがある**(2026-08-24、PXW-X70 ¥129,800):
    #   「撮影に影響するものはなく、問題なく御使用頂ける商品です。
    #     レンズ内は**小チリ程度**で、動作は専門店にて確認済みです。」
    # 限定語の後ろだけ見とったら「程度」の先に可用語が無くて撃墜しとった。
    # **直前80字**も見る。文をまたいで見るのはここだけや
    before = body[max(0, m.start() - 80): m.start()]
    if MINOR_RE.match(same) and (USABLE_RE.search(same)
                                 or USABLE_RE.search(before)):
        return False
