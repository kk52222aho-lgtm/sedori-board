...
BODY_NG = [
    ...
    ("修理歴・折れ", r"修理跡|補修跡|リペア跡|修理歴|補修歴|リペア済|折れ"),
    ...
    ("無在庫・併売", r"サンプル画像|実際に届く商品と異な|併売|他モール|"
                     r"受注後に|入荷の度に異なり|在庫確認|お届けまで\s*\d+\s*[〜~-]|"
...
DISCLAIMER_RE = re.compile(
    r"保証対象外|保証の対象外|免責|以下の事項|下記の事項|ご了承ください|"
    r"ご容赦ください|対象外となります|返品はお受けできません|返品・返金は")
DISCLAIMER_SPAN = 120  # マーカーから何字先までを免責ゾーンとみなすか
...
NEGATION_RE = re.compile(
    ...
    r"きれい|クリア|良好")
NEGATION_SPAN = 12  # 語の直後この字数以内に否定があれば無視
...
def _disclaimer_zones(body: str) -> list[tuple[int, int]]:
    return [(m.start(), m.start() + DISCLAIMER_SPAN)
            for m in DISCLAIMER_RE.finditer(body)]
...
def _is_live_hit(body: str, m: re.Match, zones: list[tuple[int, int]]) -> bool:
    ...
    return not NEGATION_RE.match(tail)
...
def judge(body: str) -> tuple[str, str]:
    ...
    hits = []
    for name, pat in BODY_NG:
        if any(_is_live_hit(body, m, zones) for m in pat.finditer(body)):
            hits.append(name)
    if hits:
